...
void EffectBase::Init(EffectData& data) {
	InitBase(data);
}

void EffectBase::InitBase(EffectData& data) {
	m_Data = data;
	m_lifeflame = 0;
	m_live = true;
	//if (data._ta_scale == Vector3{ -1, -1, -1 }) {
	//	m_scalechange = false;
	//}
	//else {
	//	m_scalechange = true;
	//	if (data.maxlife <= 0) data.maxlife = 1;//ゼロ除算防止
	//	scale_changerate = (data._ta_scale - data._scale) / data.maxlife;//1フレームあたりのスケール変化率を計算
	//}
}
